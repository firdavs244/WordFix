...
class TestAICircuitIntegration:
    ...
    def test_fallback_when_circuit_open(self):
        ...
        gemini_circuit = get_circuit("gemini")
        ...
        assert gemini_circuit.get_state() == CircuitState.OPEN
    ...
    def test_factory_switches_provider(self):
        """When groq circuit opens, factory can switch to gemini, openai, or fallback."""
        groq_circuit = get_circuit("groq")
        for _ in range(5):
            groq_circuit.record_failure()
        assert groq_circuit.get_state() == CircuitState.OPEN
        ...
        # It should be gemini, openai (if available) or fallback
        assert provider.get_provider_name() in ("gemini", "openai", "fallback")
